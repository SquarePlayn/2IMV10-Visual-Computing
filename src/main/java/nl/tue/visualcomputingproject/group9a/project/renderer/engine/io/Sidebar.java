...
public class Sidebar extends JPanel implements Camera.Listener {
	...
	public void initialize() {
		...
		renderDistanceSlider.addChangeListener(e -> {
			Settings.setChunkLoadDistance(
					Math.pow(RENDER_DISTANCE_EXPONENT_BASE, renderDistanceSlider.getValue() / 10.0));
			renderDistanceLabel.setText(String.format("%d m", (int) Settings.getChunkLoadDistance()));
		});
		c.gridx = 1;
	...
	public void updateFromCamera() {
		...
		renderDistanceSlider.setValue(
				(int) (10 * Math.log(Settings.getChunkLoadDistance()) / Math.log(RENDER_DISTANCE_EXPONENT_BASE)));
	}
